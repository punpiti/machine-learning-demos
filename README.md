...
- `conv-filter-lab/` — slide a 3×3 filter by hand over a rain-radar grid before naming it "convolution," then compare a hand-designed Sobel edge filter against one trained with the same gradient-descent update rule as `sgd-vs-lda/`, with stride and 2×2 max-pool toggles
